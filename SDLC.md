...
| Intake | A ticket or task brief is filed in github and meets the Definition of Ready below. `ot-prepare-issue` files it with SDLC labels and the ready sections; `ot-auto-manage-issues` reports what an existing ticket still lacks. | Anyone, `ot-prepare-issue`, `ot-auto-manage-issues` | Ticket exists and is ready, or its gaps are named on the ticket |
| Triage | Confirm the issue is real, still unfixed on `main`, and not already claimed or covered by an open PR. Read-only; stops the chain cleanly when there is nothing to do. | `ot-verify-in-repo` or a human | Confirmed actionable, or closed as no-action |
...
| PR | Commit, push, and open a PR against `main` with normalized labels. On a hand-worked branch, `ot-check-and-commit` runs the gate, fixes obvious drift, and pushes when green. | `ot-open-pr`, `ot-auto-create-pr`, or `ot-check-and-commit` | Open, labeled PR |
...
| Merge | `ot-merge-buddy` reports, read-only, which PRs can merge now and which are close but blocked. `ot-approve-merge-pr` re-checks every gate, approves, and squash-merges. | `ot-merge-buddy` + `ot-approve-merge-pr`, or a human | PR squash-merged into `main` |
...
When `ot-discover` has written `.ai/specs/product-brief.md`, its **Non-goals**, **Business rules**, and **Decisions** tables are protected the way `BACKWARD_COMPATIBILITY.md` protects contract surfaces. Each entry carries a stable id (`N01`, `R03`, `D07`), an owner, a status (`active` or `superseded`), a review-by date, and a required path for changing it. The rules:
...
- `risk-high` — auth, sessions, data scoping, money, schema migrations, shared contract surfaces, or broad cross-cutting edits. In this repository that means: tenant isolation and the Prisma schema (`api/prisma/`), auth and sessions (`api/src/auth/`), mail delivery (`api/src/worker/`, `src/lib/mail-*`), stored credentials and AI keys, and the API `/v1` contract.
...
- `npm run typecheck` — web app TypeScript (`tsc --noEmit`)
- `npm test` — web app unit tests (`node --test tests/*.test.mjs`)
- `npm run build` — web app production build (`next build`)
- `npm --prefix api run typecheck` — API service TypeScript
- `npm --prefix api test` — API service unit tests (`node --import tsx --test api/test/*.test.ts`)
- `npm --prefix api run build` — API service compile (`tsc -p tsconfig.json`)

Any non-zero exit fails the gate and blocks the PR. The implementing skills run the gate before opening a PR, and `ot-check-and-commit` runs it before pushing a hand-worked branch. The command list lives in `.ai/agentic.config.json`; when it changes, update it there and in this section together. The gate covers both applications in this repository — the Next.js web app at the root and the NestJS API under `api/` — and skipping the `api/` half because the diff only touched the web app is not allowed unless the config says so.

...
**Aktualny stan:** `test@sellersk.it` odbiera prawdziwe wiadomości przez IMAP. Serwer sprawdza INBOX przy uruchomieniu i co 30 sekund; panel odświeża dane co 2 sekundy. Wiadomości demonstracyjne zostały usunięte. Wysyłanie SMTP nie jest jeszcze podłączone.
...
- Trzy skrzynki: `info@panelpro.pl`, `info@sellersk.it` i `test@sellersk.it`. Odbiór IMAP działa dla ostatniej; dwie pozostałe czekają na podłączenie.

...
const expected = {
  anna: ["Anna Nowak", "Kierownik Działu Administracji i Finansów", "", "anna.nowak@opentriage.com"],
  jan: ["Jan Kowalski", "CEO | Prezes Zarządu", "", "jan.kowalski@opentriage.com"],
  michal: ["Michał Kluska", "Zespół wsparcia Open Triage", "support@example.com"],
};
...
test("trzej pracownicy mają własne podpisy o wspólnym układzie", () => {
  for (const [id, values] of Object.entries(expected)) {
    ...
    assert.ok(signatureText(signature).includes("Open Triage Spółka z ograniczoną odpowiedzialnością"));
  }
});

test("HTML podpisu koduje treść wiadomości i nie pobiera zewnętrznych obrazów", async () => {
  const html = await messageHtml('<script>alert("x")</script>\nDruga linia\n</mj-text><mj-include path=".env.local" />', employeeSignatures.anna);
...
test("rezerwacja utrwala podpis autora, a MIME zawiera wersję tekstową i HTML", async () => {
  const signature = structuredClone(employeeSignatures.anna);
  const state = {
    generation: "g", appliedRequests: [], users: [{ id: "anna", name: signature.name, role: signature.title, signature }],
    ...
    drafts: [{ key: "c:anna:reply", text: "Treść", version: 1, basePublicRevision: 1 }], outbox: [],
  };
  const job = reserveReply(state, { requestId: "request", generation: "g", userId: "anna", action: { conversationId: "c", text: "Treść", expectedPublicRevision: 1, draftVersion: 1 } });
  state.users[0].signature.name = "Zmienione później";
  assert.equal(job.email.signature.name, "Anna Nowak");
  const mail = await simpleParser(await composeReply(job));
  assert.match(mail.text, /Treść[\s\S]*Anna Nowak[\s\S]*729 921 970/);

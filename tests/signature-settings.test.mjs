...
test("edytowalny MJML: walidacja, trwałość, konflikty i podpis utrwalony w MIME", async () => {
  ...
  try {
    ...
    const source = defaultSignatureMjml(employeeSignatures.irena).replace("729 921 970", "111 222 333").replace("Kierownik Działu Administracji i Finansów", "Nowe stanowisko");
    const input = { userId: "irena", actorId: "tomasz", generation: state.generation, expectedVersion: 0, mjml: source };
    ...
    assert.equal((await getState()).users.find((u) => u.id === "irena").signature.custom, undefined);
    const first = await saveUserSignature(input);
    assert.equal(first.version, 1);
    assert.equal(first.updatedBy, "tomasz");
    const restored = (await getState()).users.find((u) => u.id === "irena");
    ...
    assert.equal(disk.users.find((u) => u.id === "irena").signature.custom.version, 1);
    ...
    assert.equal((await getState()).users.find((u) => u.id === "irena").signature.custom.version, 1);
    ...
    const frozen = signatureFor(restored, "test@sellersk.it");
    ...
    const mime = await simpleParser(await composeReply({
      subject: "Test podpisu", email: { from: "test@sellersk.it", to: "nobody@example.test", body: "Treść odpowiedzi",

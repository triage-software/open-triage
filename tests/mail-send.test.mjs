...
function fixture() {
  const state = {
    ...
    users: [{ id: "tomasz", name: "Tomasz Dłuski", active: true }],
    ...
    drafts: [{ key: "thread:tomasz:reply", text: "Dzień dobry, odpowiedź zespołu.", version: 1, basePublicRevision: 1 }],
  };
  const request = {
    requestId: "request-1", generation: state.generation, userId: "tomasz",
...
test("wspólny From i Reply-To, nagłówki wątku, komentarze poza MIME", async () => {
  ...
  assert.equal(parsed.from.value[0].address, "test@sellersk.it");
  assert.equal(parsed.replyTo.value[0].address, "test@sellersk.it");
  ...
  assert.equal(parsed.messageId, "<request-1@sellersk.it>");
  ...
  assert.match(parsed.text, /Tomasz Dłuski/);
  assert.match(parsed.text, /test@sellersk\.it/);
  assert.match(parsed.html, /SellersKit · Pryzmat Media/);
  assert.ok(!raw.toString().includes("TAJNY KOMENTARZ"));
  assert.ok(!raw.toString().includes("PYTANIE KLIENTA"));
  assert.equal(job.email.authorName, "Tomasz Dłuski");
});

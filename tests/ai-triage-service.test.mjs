...
const mail = (id, references = []) => ({ subject: "Licencja", email: {
  ...
  from: "client@example.test", to: "test@sellersk.it", body: `Licencja ${id}`, createdAt: new Date().toISOString(),
} });
...
test("automatyczna klasyfikacja po odbiorze — izolowany IMAP, AI i trwały zapis", async (t) => {
  ...
  globalThis.triageTestImap = class {
    ...
    async fetchOne() { return { source: Buffer.from([
      "From: Klient <client@example.test>", "To: test@sellersk.it", "Subject: Licencja",
  ...
  try {
    ...
    process.env.TRIAGE_IMAP_USER = "test@sellersk.it";
